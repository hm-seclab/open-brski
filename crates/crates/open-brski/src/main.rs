...
async fn main() -> anyhow::Result<(), AppError> {
    ...
    if matches!(cli.command, cli::Command::TestCerts) {
        ...
        example_certs::serialize_certs(certs, current_dir().unwrap().join("reference_keys"));
        return Ok(());
    }

    let mut tasks: Vec<JoinHandle<_>> = match &cli.command {
        cli::Command::RegistrarAgent(_) => vec![registrar_agent::start(config.registrar_agent)
            .await
            .unwrap()],
        ...
        cli::Command::All => {
            vec![
                registrar_agent::start(config.registrar_agent)
                    .await
                    .unwrap(),
