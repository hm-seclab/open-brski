...
impl Validate for MasaConfig {
    fn validate(&self) -> anyhow::Result<()> {
        ...
        if !self.ca_certificate.relative().exists() {
            return Err(anyhow!(
                "masa ca_certificate is empty or not exist".to_owned()
            ));
        }
        ...
        if !self.masa_certificate.relative().exists() {
            return Err(anyhow!(
                "masa ee_certificate is empty or not exist".to_owned()
            ));
        }
        ...
        if !self.registrar_ee_certificate.relative().exists() {
            return Err(anyhow!(
                "registrar ee_certificate is empty or not exist".to_owned()
            ));
        }
        Ok(())
    }
...
impl Default for MasaConfig {
    fn default() -> Self {
        Self {
            ...
            masa_key: RelativePathBuf::from(
                "/etc/open-brski/conf/masa/signing-authority/vendor.key",
            ),
